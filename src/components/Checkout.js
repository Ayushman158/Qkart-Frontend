...
const Checkout = () => {
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try{
      ...
      if(e.response) {
        enqueueSnackbar(e.reponse.data.message, {variant:"error"});
      } else {
        enqueueSnackbar("Could not place order. Check that backend is running, reachable and returns valid JSON.",
        {
          variant: "error",
        }
        );
